...
<x-app-layout>
    ...
    <div class="container mx-auto mt-4">
        <x-success/>
        <x-delete-button>{{route('answers.destroy', $answer->id)}}</x-delete-button>
            ...
            <div>
                ...
                <select name="type_id" id="type_id" class="w-full rounded-md border border-gray-300 px-4 py-2 focus:outline-none focus:border-blue-500 @error('type_id') border-red-500 @enderror" required>
                    ...
                    @foreach($types as $type)
                        <option value="{{ $type->id }}" @if($type->id === $answer->type_id) selected @endif>{{ $type->name }}</option>
                    @endforeach
                </select>
                @error('type_id')
                ...
                <div>
                    ...
                    <div>
                        <label for="question_{{$index + 1}}_1" class="flex items-center">
                            <input type="radio" id="question_{{$index + 1}}_1" name="question_{{$index + 1}}" value="1" class="form-radio h-4 w-4 text-blue-600" @if($answer['question_'.($index + 1)] === '1') checked @endif>
                            <span class="ml-2">{{$question->option_1}}</span>
                        </label>
                    </div>
                    <div>
                        <label for="question_{{$index + 1}}_2" class="flex items-center">
                            <input type="radio" id="question_{{$index + 1}}_2" name="question_{{$index + 1}}" value="2" class="form-radio h-4 w-4 text-blue-600" @if($answer['question_'.($index + 1)] === '2') checked @endif>
